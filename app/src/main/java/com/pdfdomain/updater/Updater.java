...
public class Updater {
    ...
    public void checkUpdate(){
        ApiClient.getInstance().getUpdateJSON().enqueue(new Callback<JsonObject>() {
            ...
            public void onResponse(Call<JsonObject> call, retrofit2.Response<JsonObject> response) {
                ...
                try{
                    newVersionCode = "1";//response.body().getAsJsonObject("hits").get("total").getAsJsonObject()
                            //.get("value").getAsString();
                    currentVersionCode = String.valueOf(1);

                    updateURL = "test";//response.body().getAsJsonObject("update").get("url").getAsString();
    ...
    public void updateApp() {
        ...
        if (isNetworkAvailable()) {
            String APK_FILENAME = "pdfdomain-update.apk";
